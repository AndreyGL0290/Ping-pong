...
function loop(){
    ...
    if (ball.x < leftPaddle.x || ball.x > rightPaddle.x){
        if (ball.x < leftPaddle.x){
            win_counter1 += 1;
            document.getElementById('player_1').textContent = "Побед у второго игрока " + win_counter1;
        }
        else if (ball.x > rightPaddle.x){
            win_counter2 += 1;
            document.getElementById('player_2').textContent = "Побед у первого игрока " + win_counter2;
        }
